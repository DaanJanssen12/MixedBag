function buildEvolutionChart(evolutions){
    ...
    evolutions.forEach(evolution => {
        ...
        var $a = $("<a></a>").attr("href", `index.html?module=pokemon&page=pokedex&id=${evolution.nr}_${evolution.name.toLowerCase()}`);
        var $picture = $("<picture></picture>");
        var $img = $("<img></img>").addClass("img-fixed").attr("src", `data/pokemon/pokemon/img/${evolution.nr}_${evolution.name.toLowerCase()}.png`);
        ...
        if(evolution.alternateEvo){
            ...
            var $altA = $("<a></a>").attr("href", `index.html?module=pokemon&page=pokedex&id=${evolution.alternateEvo.nr}_${evolution.alternateEvo.name.toLowerCase()}`);
            var $altPicture = $("<picture></picture>");
            var $altImg = $("<img></img>").addClass("img-fixed").attr("src", `data/pokemon/pokemon/img/${evolution.alternateEvo.nr}_${evolution.alternateEvo.name.toLowerCase()}.png`);
...
$(document).ready(function(){
    ...
    if(fireOnce <= 0){
        ...
        var pokemon = 'data/pokemon/pokemon/' + id + '.json';
        readTextFile(pokemon, function(text){
            ...
            var imgSrc = "data/pokemon/pokemon/img/"+id+".png";
            ...
            var movesDB = null;
            readTextFile("data/pokemon/moves.json", function(moveText){
            ...
            readTextFile("data/pokemon/tms.json", function(tmText){
        ...
        readTextFile("data/pokemon/pokedex-full.json", function(text){
            var data = JSON.parse(text);
            ...
            var pokemon = data[nr];
            var $a = $("<a></a>").attr("href", `index.html?module=pokemon&page=pokedex&id=${pokemon.nr}_${pokemon.name.toLowerCase()}`).text(`< #${pokemon.nr} - ${pokemon.name}`);
            $("#previous").html($a);
            ...
            var $aNext = $("<a></a>").attr("href", `index.html?module=pokemon&page=pokedex&id=${pokemonNext.nr}_${pokemonNext.name.toLowerCase()}`).text(`#${pokemonNext.nr} - ${pokemonNext.name} >`);
            $("#next").html($aNext);
        });
