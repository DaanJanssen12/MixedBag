...
function getEvolutionPBSDataString(pokemon) {
    ...
    var evosAfterSelf = pokemon.evolutions.filter(f => f.lvl > self.lvl || (f.name !== self.name && f.lvl === -1) || (f.lvl == -1 && f.lvl !== self.lvl));
...
function getEvolutionParamPBSString(evolution) {
    if (evolution.extraInfo !== null && evolution.extraInfo !== undefined) {
        if (evolution.extraInfo.includes('Stone')) {
            return replaceAll(evolution.extraInfo, ' ', '').toUpperCase();
        }
    }
